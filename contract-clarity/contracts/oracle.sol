...
contract ChatGpt {
    ...
    event ChatUpdated(uint indexed chatId, string response);
    ...
    function startChat(string memory smartContractAddress) public returns (uint) {
        ChatRun storage run = chatRuns[chatRunsCount];

        run.owner = msg.sender;
        Message memory newMessage;
        newMessage.content = smartContractAddress;
    ...
    function onOracleLlmResponse(uint runId, string memory response, string memory /*errorMessage*/) public onlyOracle {
        ...
        run.messages.push(newMessage);
        run.messagesCount++;
        emit ChatUpdated(runId, response);
    }
}